...
import saabBundled from "../../public/data/saab.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/saab.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const SAAB_DATA_FALLBACK = saabBundled as IaiPayload;
...
export function SaabCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/saab.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/saab.json", { cache: "no-store" })
      .then((r) => {
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — סאאב</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/saab.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:saab</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/saab-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:saab</code>.
      </div>
      ...
      {latest ? (
        <div className="card-grid">
          ...
          <div className="card">
            <div className="label">הכנסות (מיליון MSEK)</div>
            <div className="value">{fmtNum(latest.revenueMUSD)}</div>
          </div>
          <div className="card">
            <div className="label">מלאי הזמנות (מיליון MSEK)</div>
            <div className="value">{fmtNum(latest.backlogMUSD)}</div>
          </div>
          <div className="card">
            <div className="label">רווח נקי (מיליון MSEK)</div>
            <div className="value">{fmtNum(latest.netIncomeMUSD)}</div>
          </div>
          <div className="card">
            <div className="label">רווח תפעולי — EBIT (מיליון MSEK)</div>
          ...
          <div className="card">
            <div className="label">EBITDA (מיליון MSEK)</div>
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/saab-metrics.json</span> והרץ{" "}
          <code>npm run data:saab</code>.
        </p>
      )}
      {latest ? (
        <>
          <h3 className="section-subh">תזרים, מאזן והשקעה (מיליון MSEK)</h3>
          ...
          <div className="card-grid">
            ...
            <div className="card">
              ...
              <p className="muted" style={{ fontSize: "0.72rem", margin: "0.35rem 0 0" }}>
                תזרים תפעולי − CapEx (מיליון MSEK)
              </p>
            </div>
            <div className="card">
      ...
      {(cashFlowChartHasData ||
        ...
        <>
          ...
          <p className="muted small">
            כל הערכים במיליון MSEK. תזרים השקעה ומימון לעיתים שליליים לפי דיווח.
          </p>
          {cashFlowChartHasData && (
            <>
      ...
      {segmentKeys.length > 0 && (
        <>
          <h2>פילוח הכנסות לפי מגזר (מיליון MSEK)</h2>
          <p className="muted small">
