...
import rheinmetallBundled from "../../public/data/rheinmetall.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/rheinmetall.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const RHEINMETALL_DATA_FALLBACK = rheinmetallBundled as IaiPayload;
...
export function RheinmetallCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/rheinmetall.json ומשתמשים בעותק מהבילד */
    ...
    fetch("/data/rheinmetall.json", { cache: "no-store" })
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setData(RHEINMETALL_DATA_FALLBACK);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — Rheinmetall</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/rheinmetall.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:rheinmetall</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/rheinmetall-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:rheinmetall</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/rheinmetall-metrics.json</span> והרץ{" "}
          <code>npm run data:rheinmetall</code>.
        </p>
      )}
      {latest ? (
        <>
