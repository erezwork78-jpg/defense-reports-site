...
import northropBundled from "../../public/data/northrop.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/northrop.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const NORTHROP_DATA_FALLBACK = northropBundled as IaiPayload;
...
export function NorthropCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/northrop.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/northrop.json", { cache: "no-store" })
      .then((r) => {
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setData(NORTHROP_DATA_FALLBACK);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — נורת׳רופ גראמן</p>
      ...
      <div className="disclaimer">
        ...
        <span className="file-path">public/data/northrop-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:northrop</code>.
      </div>
