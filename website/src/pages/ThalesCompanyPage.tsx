...
import thalesBundled from "../../public/data/thales.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/thales.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const THALES_DATA_FALLBACK = thalesBundled as IaiPayload;
...
export function ThalesCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/thales.json ומשתמשים בעותק מהבילד */
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — Thales</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/thales.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:thales</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/thales-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:thales</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/thales-metrics.json</span> והרץ{" "}
          <code>npm run data:thales</code>.
        </p>
      )}
      {latest ? (
        <>
