...
import leonardoBundled from "../../public/data/leonardo.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/leonardo.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const LEONARDO_DATA_FALLBACK = leonardoBundled as IaiPayload;
...
export function LeonardoCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/leonardo.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/leonardo.json", { cache: "no-store" })
      .then((r) => {
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setData(LEONARDO_DATA_FALLBACK);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — לאונרדו</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/leonardo.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:leonardo</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/leonardo-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:leonardo</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/leonardo-metrics.json</span> והרץ{" "}
          <code>npm run data:leonardo</code>.
        </p>
      )}
      {latest ? (
        <>
