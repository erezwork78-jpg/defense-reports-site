...
import l3harrisBundled from "../../public/data/l3harris.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/l3harris.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const L3HARRIS_DATA_FALLBACK = l3harrisBundled as IaiPayload;
...
export function L3harrisCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/l3harris.json ומשתמשים בעותק מהבילד */
    ...
    fetch("/data/l3harris.json", { cache: "no-store" })
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setData(L3HARRIS_DATA_FALLBACK);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — L3Harris</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/l3harris.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:l3harris</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/l3harris-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:l3harris</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/l3harris-metrics.json</span> והרץ{" "}
          <code>npm run data:l3harris</code>.
        </p>
      )}
      {latest ? (
        <>
