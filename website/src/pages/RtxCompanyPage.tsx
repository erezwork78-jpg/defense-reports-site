...
import rtxBundled from "../../public/data/rtx.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/rtx.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const RTX_DATA_FALLBACK = rtxBundled as IaiPayload;
...
export function RtxCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/rtx.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/rtx.json", { cache: "no-store" })
      .then((r) => {
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — RTX</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/rtx.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:rtx</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/rtx-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:rtx</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/rtx-metrics.json</span> והרץ{" "}
          <code>npm run data:rtx</code>.
        </p>
      )}
      {latest ? (
        <>
