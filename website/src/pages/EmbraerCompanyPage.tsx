...
import embraerBundled from "../../public/data/embraer.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/embraer.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const EMBRAER_DATA_FALLBACK = embraerBundled as IaiPayload;
...
export function EmbraerCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/embraer.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/embraer.json", { cache: "no-store" })
      .then((r) => {
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — אמבראיר</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/embraer.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:embraer</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/embraer-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:embraer</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/embraer-metrics.json</span> והרץ{" "}
          <code>npm run data:embraer</code>.
        </p>
      )}
      {latest ? (
        <>
