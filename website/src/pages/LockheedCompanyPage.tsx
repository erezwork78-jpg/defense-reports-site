...
const LOCKHEED_DATA_FALLBACK = lockheedBundled as IaiPayload;
...
export function LockheedCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/lockheed.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/lockheed.json", { cache: "no-store" })
      .then((r) => {
      ...
      .catch(() => {
        if (!cancelled) {
          ...
          setData(LOCKHEED_DATA_FALLBACK);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — לוקהיד מרטין</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/lockheed.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:lockheed</code>.
        </div>
      )}
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/lockheed-metrics.json</span> והרץ{" "}
          <code>npm run data:lockheed</code>.
        </p>
      )}
      {latest ? (
        <>
