...
import rafaelBundled from "../../public/data/rafael.json";
import { SiteNav } from "../components/SiteNav";

/** כש־fetch ל־/data/rafael.json נכשל (file://, שרת בלי public), מציגים את העותק מהבילד */
const RAFAEL_DATA_FALLBACK = rafaelBundled as IaiPayload;
...
export function RafaelCompanyPage() {
  const [data, setData] = useState<IaiPayload | null>(null);
  const [err, setErr] = useState<string | null>(null);
  /** true אם לא הצלחנו לטעון מ־/data/rafael.json ומשתמשים בעותק מהבילד */
  const [fromBundledFallback, setFromBundledFallback] = useState(false);

  useEffect(() => {
    let cancelled = false;
    fetch("/data/rafael.json", { cache: "no-store" })
      .then((r) => {
  ...
  return (
    <>
      <SiteNav />
      <header>
        <p className="badge">דוחות כספיים — רפאל</p>
      ...
      {fromBundledFallback && (
        <div className="disclaimer" role="status">
          מוצגים נתונים מעותק מוטמע (לא נטען הקובץ <span className="file-path">/data/rafael.json</span>).
          הרץ <code>npm run dev</code> מתוך תיקיית <code>website</code> או שרת סטטי שמגיש את תיקיית{" "}
          <code>dist</code> כולה — כדי לראות את הקובץ העדכני אחרי <code>npm run data:rafael</code>.
        </div>
      )}

      <div className="disclaimer">
        הנתונים המספריים והתובנות הידניות נשמרים ב־
        <span className="file-path">public/data/rafael-metrics.json</span> (מפתחות{" "}
        <code>rows</code>, <code>insights</code>) — הוראות בעמוד <strong>README</strong>{" "}
        בתיקיית האתר. רשימת הקבצים נוצרת עם <code>npm run data:rafael</code>.
      </div>
      ...
      {latest ? (
        ...
        <p className="muted">
          ...
          <span className="file-path">public/data/rafael-metrics.json</span> והרץ{" "}
          <code>npm run data:rafael</code>.
        </p>
      )}
      {latest ? (
        <>
